...
import ProfileStats from '../Blocks/Profile/ProfileStats'
...
const Canvas = (props) => {
  const {blocks, themeStyle, selectedPreviewMenu, isSettingsEnabled, isInserterEnabled, getBlocks} = props
      ...
      <div
        ...
        { blocks && Object.keys(blocks).map(key => {
          ...
          switch (block.type) {
            ...
            case 'BUTTON_ICON':
              return <ButtonIcon key={ key } _id={ key } themeStyle={ themeStyle['.button-with-icon-component'] } block={ block }/>
