...
import { getProjectNotif } from '../../stores/actions';

import './Notif.css';

const Notif = ({ refNotifMenu, openNotifMenu, getProjectNotif, notif }) => {
    let { id } = useParams();
    ...
    const showNotifList = () => {
        return(<div className='notif-list-container'>
            { notif[id].sort().map((item, i) => <div className='notif-list-item' key={i}>
                ...
                <div className='notif-item-action'>
                    { item.next_action ? <button className='notif-action-button' onClick={() => goToURL(item.next_source_url)}>{item.next_action}</button> : 
                        <Loader
                            type="Oval"
                            color="#00BFFF"
                            height={18}
    ...
    return (<div className="notif-list-items" ref={refNotifMenu} style={{display: openNotifMenu ? "block" : "none"}}>
                ...
                <div className="list-content">
                    { notif[id] && notif[id].length > 0 ? showNotifList() : showEmptyNotifList() }
                </div>
            </div>);
}
